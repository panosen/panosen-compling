...
using Panosen.Compling.SLR1.Rules;
...
namespace Panosen.Compling.Display
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            Prepare(new SlrSampleRule1().GetRules());

            this.tbInput.Text = new SlrSampleRule1().GetSamples();
        }

        private void BtnCase2_Click(object sender, RoutedEventArgs e)
        {
            Prepare(new SampleRule2().GetRules());

            this.tbInput.Text = new SampleRule2().GetSamples();
        }

        private void BtnCase3_Click(object sender, RoutedEventArgs e)
        {
            var sampleRuleThree = new SlrSampleRuleThree();
            Prepare(sampleRuleThree.GetRules());

            this.tbInput.Text = sampleRuleThree.GetSamples();
        }

        private void BtnCase4_Click(object sender, RoutedEventArgs e)
        {
            Prepare(new SampleRule4().GetRules());
        }

        private void BtnCase5_Click(object sender, RoutedEventArgs e)
        {
            Prepare(new SampleRule5().GetRules());

            this.tbInput.Text = new SampleRule5().GetSamples();
        }

        private void BtnCase6_Click(object sender, RoutedEventArgs e)
        {
            Prepare(new SlrSampleRule6().GetRules());

            this.tbInput.Text = new SlrSampleRule6().GetSamples();
        }
        ...
        private void Prepare(List<ProductionRule> rules)
        {
            ...
            {
                listviewP.Items.Add(new ListViewItem() { Content = ToString(rule) });
            }
            ...
            {
                listviewStates.Items.Add(new { Name = state.Name, Productions = string.Join("  ", dfa.StateRuleListMap[state].Select(v => ToString(v))) });
            }
        ...
        private static string ToString(ProductionRule rule)
        {
            StringBuilder sb = new StringBuilder();
